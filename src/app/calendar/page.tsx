...
export default function CalendarPage() {
  ...
  const yearLabel = monthLabel.split(' ').slice(1).join(' ');
    ...
    return (
      <AppLayout userRole={user?.role} userName={user?.name}>
        <div className="min-h-[60vh] flex items-center justify-center text-bone-2 font-mono text-sm">
          <span className="ap-pulse-dot">cargando calendario…</span>
        </div>
      </AppLayout>
    );
  }
      ...
      <div className="px-6 lg:px-12 py-8 lg:py-12 max-w-[1280px] mx-auto">
        ...
        <header className="ap-fade-up mb-8 lg:mb-10">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
            <div>
              <p className="eyebrow mb-3">Calendario · vista mensual</p>
              <h1 className="font-display tracking-editorial text-[clamp(2.25rem,4.6vw,4rem)] leading-[0.95]">
                <span className="capitalize">{monthName}</span>{' '}
                <span className="italic text-bone-2">{yearLabel}</span>
